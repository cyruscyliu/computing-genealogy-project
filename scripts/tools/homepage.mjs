...
function sanitizeSchoolLabel(value) {
  ...
  trimmed = trimmed
    ...
    .replace(
      /\s+with\s+(?:(?:an?\s+)?(?:m\.?s\.?|b\.?s\.?|master'?s|bachelor'?s)\b.*)$/i,
      ""
    )
...
export function detectProfileSignalsFromText(text) {
  ...
  const advisorPatterns = [
    ...
    /\b(?:fortunate|lucky)\s+to\s+be\s+advised\s+by\s+(.+?)(?:,\s+as\s+(?:a|an)\b|[.!?;]|$)/i,
    ...
    /\badvised by\s+(.+?)(?:\s+as\s+part\s+of\b|,\s+and\s+(?:completed|spent|supported|funded)\b|\s+and\s+(?:completed|spent|was|were|supported|funded)\b|[.!?;]|$)/i,
  ...
  const hasExplicitDoctoralAdvisorContext = (sentence) =>
    /\b(?:ph\.?d|doctoral|graduate research|thesis|dissertation|my\s+(?:primary\s+)?phd\s+advisor)\b/i.test(
      sentence ?? ""
    );
  ...
  if (!phdAdvisorLabel) {
    const firstPersonAdvisorPatterns = [
      ...
      /\b(?:fortunate|lucky)\s+to\s+be\s+advised\s+by\s+(.+?)(?:,\s+as\s+(?:a|an)\b|[.!?;]|$)/i,
      /\bi\s+was\s+advised\s+by\s+(.+?)(?:\s+as\s+part\s+of\b|[.!?;]|$)/i,
      /\bi\s+was\s+supervised\s+by\s+(.+?)(?:[.!?;]|$)/i,
      /\bboth with\s+(.+?)(?:[.!?;]|$)/i,
    ];
    for (const sentence of sentences.filter(hasExplicitDoctoralAdvisorContext)) {
